...
int main()
{
    ...
    game.AddItem(objects::Coin(7,3,7));
    ...
    while (window.isOpen())
    {
        ...
        for(int i = 0; i < field.GetHeight();i++){
            for(int j = 0; j < field.GetWidth();j++){
                objects::Cell& cell = field.GetCell(i,j);
                switch (cell.GetType())
                {
        ...
        while(item){
            ...
            sprite.setPosition(item->GetWidthPosition()*SIZE, item->GetHeightPosition()*SIZE);
            window.draw(sprite);
            i++;
            item = game.GetItem(i);
        }
        sprite.setTexture(textures[4]);
        sprite.setPosition(player.GetWidthPosition()*SIZE, player.GetHeightPosition()*SIZE);
        window.draw(sprite);
